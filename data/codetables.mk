...
ABILITY_FLAG_PARAMS_TARGET := $(BUILD)/a028/9_19
ABILITY_FLAG_PARAMS_DEPENDENCIES := data/AbilityFlags.c
ABILITY_FLAG_PARAMS_OBJS := $(patsubst data/%.c,build/%.o,$(ABILITY_FLAG_PARAMS_DEPENDENCIES))
ABILITY_FLAG_PARAMS_BIN := $(patsubst data/%.c,build/%.bin,$(ABILITY_FLAG_PARAMS_DEPENDENCIES))

$(ABILITY_FLAG_PARAMS_BIN): $(ABILITY_FLAG_PARAMS_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(ABILITY_FLAG_PARAMS_OBJS)
	$(OBJCOPY) -O binary $(ABILITY_FLAG_PARAMS_OBJS) $@

NARC_FILES += $(ABILITY_FLAG_PARAMS_BIN)


END_OF_MAIN_BRANCH_CODETABLE_NUMBER :=19

TOTEMBATTLES_TARGET := $(BUILD)/a028/9_$(shell expr $(END_OF_MAIN_BRANCH_CODETABLE_NUMBER) + 1 )
TOTEMBATTLES_DEPENDENCIES := data/TotemBattles.c
TOTEMBATTLES_OBJS := $(patsubst data/%.c,build/%.o,$(TOTEMBATTLES_DEPENDENCIES))
TOTEMBATTLES_BIN := $(patsubst data/%.c,build/%.bin,$(TOTEMBATTLES_DEPENDENCIES))

$(TOTEMBATTLES_BIN): $(TOTEMBATTLES_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(TOTEMBATTLES_OBJS)
	$(OBJCOPY) -O binary $(TOTEMBATTLES_OBJS) $@

NARC_FILES += $(TOTEMBATTLES_BIN)

MAXRAIDBATTLES_TARGET := $(BUILD)/a028/9_$(shell expr $(END_OF_MAIN_BRANCH_CODETABLE_NUMBER) + 2 )
MAXRAIDBATTLES_DEPENDENCIES := data/MaxRaidBattles.c
MAXRAIDBATTLES_OBJS := $(patsubst data/%.c,build/%.o,$(MAXRAIDBATTLES_DEPENDENCIES))
MAXRAIDBATTLES_BIN := $(patsubst data/%.c,build/%.bin,$(MAXRAIDBATTLES_DEPENDENCIES))

$(MAXRAIDBATTLES_BIN): $(MAXRAIDBATTLES_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(MAXRAIDBATTLES_OBJS)
	$(OBJCOPY) -O binary $(MAXRAIDBATTLES_OBJS) $@

NARC_FILES += $(MAXRAIDBATTLES_BIN)

TERARAIDBATTLES_TARGET := $(BUILD)/a028/9_$(shell expr $(END_OF_MAIN_BRANCH_CODETABLE_NUMBER) + 3 )
TERARAIDBATTLES_DEPENDENCIES := data/TeraRaidBattles.c
TERARAIDBATTLES_OBJS := $(patsubst data/%.c,build/%.o,$(TERARAIDBATTLES_DEPENDENCIES))
TERARAIDBATTLES_BIN := $(patsubst data/%.c,build/%.bin,$(TERARAIDBATTLES_DEPENDENCIES))

$(TERARAIDBATTLES_BIN): $(TERARAIDBATTLES_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(TERARAIDBATTLES_OBJS)
	$(OBJCOPY) -O binary $(TERARAIDBATTLES_OBJS) $@

NARC_FILES += $(TERARAIDBATTLES_BIN)

BOSSBATTLES_TARGET := $(BUILD)/a028/9_$(shell expr $(END_OF_MAIN_BRANCH_CODETABLE_NUMBER) + 4 )
BOSSBATTLES_DEPENDENCIES := data/BossBattles.c
BOSSBATTLES_OBJS := $(patsubst data/%.c,build/%.o,$(BOSSBATTLES_DEPENDENCIES))
BOSSBATTLES_BIN := $(patsubst data/%.c,build/%.bin,$(BOSSBATTLES_DEPENDENCIES))

$(BOSSBATTLES_BIN): $(BOSSBATTLES_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(BOSSBATTLES_OBJS)
	$(OBJCOPY) -O binary $(BOSSBATTLES_OBJS) $@

NARC_FILES += $(BOSSBATTLES_BIN)
