...
static const u16 SoundProofMovesList[] = {
        MOVE_BOOMBURST,
        MOVE_BUG_BUZZ,
        MOVE_CHATTER,
                   ...
                   u32 field_cond, u16 pow, u8 type UNUSED, u8 attacker, u8 defender, u8 critical)
{
    ...
    if ((AttackingMon.ability == ABILITY_TOXIC_BOOST) && (AttackingMon.condition & STATUS_POISON_ALL) && (movetype == TYPE_POISON))
    {
    ...
    if ((AttackingMon.ability == ABILITY_FLARE_BOOST) && ((AttackingMon.condition & STATUS_BURN)) && (movetype == TYPE_FIRE))
    {
        movepower = movepower * 150 / 100;
    }
...
/*    // if dark aura is present but not aura break
    ...
    if (AttackingMon.ability == ABILITY_UNAWARE || moveno == MOVE_SACRED_SWORD)
    {
        defstate = 0;
        spdefstate = 0;
    }
